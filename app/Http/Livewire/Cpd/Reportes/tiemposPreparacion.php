...
class tiemposPreparacion extends Component
{
    public $pasos;
    public $iconos;
    ...
    public function render()
    {
        ...
        $tareasPreparacion = CpdTipos::select('ID')->where('Requerimiento','Preparacion')->get()->toArray();
        $casos = [];
        $this->pasos = CpdTipos::where('Requerimiento','Preparacion')->get();

        $flujosCpd = CpdTareas::selectRaw('CpdID, CPD_Tareas.ID, SucursalID, CPD_Tareas.created_at')
            ->join('CPD_Flujos','CPD_Flujos.ID','=','CPD_Tareas.CpdID')
            ->whereIn('CPD_Tareas.Estado',['Abierto','Postergado'])
            ->whereIn('CPD_Tareas.CpdTipoID',$tareasPreparacion)
        ...
        foreach ($flujosCpd as $flujo) {
            ...
            $casos[$flujo->ID]["etapa"] = $tarea->Tipo->ID;
        ...
        return view('livewire.cpd.reportes.tiempos-preparacion');
    }
    ...
    public function updateDatatable($searchGerencia,$searchSucursal,$inputFechaInicio,$inputFechaFin): void
    {
        ...
        $this->emitTo('tiempos-preparacion', 'refreshComponent');
