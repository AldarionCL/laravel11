<form name="formTarea" action="{{route('completartareatdrive',[$tarea->ID])}}" method="post" id="formTarea" class=" p-2 border border-gray-200 rounded-lg dark:border-gray-700 m-0 shadow-md" enctype="multipart/form-data">

{{--
<form name="formTarea" wire:submit.prevent="completartareatdrive" type="post" enctype="multipart/form-data">
--}}
    @csrf
        ...
        <div class="w-full sm:w-100 md:w-1/2 lg:w-2/3 xl:w-2/3">
            ...
            @foreach($tarea->Tipo->Campos as $campo)
                <div class="border border-light mb-3 p-2 border-gray-300">
                    <label for="{{$campo->InputName}}" class="border-b-2 border-blue-300 block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{$campo->Campo}}</label>
                    @if($campo->Tipo == 'info')
                        <p id="{{$campo->InputName}}" name="{{$campo->InputName}}" class="{{$campo->Clase}}  border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5  dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" >{{$campo->VarOption}}</p>
                    @endif
                    @if($campo->Tipo == 'text')
                        <input type="text" id="{{$campo->InputName}}" name="{{$campo->InputName}}" class="{{$campo->Clase}} bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="{{$campo->Placeholder}}" {{($campo->Requerido) ? 'required' : ''}}>
                    @endif
                    @if($campo->Tipo == 'number')
                        <input type="number" id="{{$campo->InputName}}" name="{{$campo->InputName}}" step="0.01" class="{{$campo->Clase}} bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="{{$campo->Placeholder}}" {{($campo->Requerido) ? 'required' : ''}} value="0">
                    @endif
                    @if($campo->Tipo == 'checkbox')
                        <input id="{{$campo->InputName}}" name="{{$campo->InputName}}" type="checkbox" value="{{($campo->VarOption != null) ? $campo->VarOption : 1}}" name="bordered-checkbox"
                               class=" {{$campo->Clase}} w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" {{($campo->Requerido) ? 'required' : ''}}>
                    @endif
                    @if($campo->Tipo == 'date')
                        <input type="datetime-local" id="{{$campo->InputName}}" name="{{$campo->InputName}}" value="{{date('Y-m-d H:i')}}" class="{{$campo->Clase}} bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="{{$campo->Placeholder}}" {{($campo->Requerido) ? 'required' : ''}}>
                    @endif
                    @if($campo->Tipo == 'textarea')
                        <textarea id="{{$campo->InputName}}" name="{{$campo->InputName}}" rows="4" class="{{$campo->Clase}} bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="{{$campo->Placeholder}}" {{($campo->Requerido) ? 'required' : ''}}></textarea>
                    @endif
                    @if($campo->Tipo == 'file')
                        <input id="{{$campo->InputName}}" name="{{$campo->InputName}}"  type="file" class="{{$campo->Clase}} bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="{{$campo->Placeholder}}" {{($campo->Requerido) ? 'required' : ''}} >
                    @endif
                    @if($campo->Tipo == 'files')
                        <input id="{{$campo->InputName}}" name="{{$campo->InputName}}[]"  type="file" class="{{$campo->Clase}} bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="{{$campo->Placeholder}}" {{($campo->Requerido) ? 'required' : ''}} multiple>
                    @endif
                    ...
                    @if($campo->Tipo == 'select')
                        ...
                        <x-simple-select
                            ...
                            :searchable="true"
                            class="{{$campo->Clase}} bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                            no-options="No hay datos"
                        />
                    @endif
                </div>
            @endforeach
            @if($tarea->TdriveTipoID == 23 || $tarea->TdriveTipoID == 3)
                <div class="border border-light mb-3 p-2 border-gray-300">
                    <livewire:tdrive.componentes.modales.taller-grid : idSucursal="{{$tdrive->SucursalID}}" idTdrive="{{$tarea->TdriveID}}"/>
                </div>
            @endif
        </div>
