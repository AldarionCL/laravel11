<form id="formLog" name="formLog"  method="get" action="{{route('creaLogDYP',[$idDyp])}}"
class="p-3">
    ...
    <ul class="w-100 text-xs font-mediartinez Sotoum text-gray-900 bg-white border border-gray-200 rounded-lg">
        <li class="w-full px-4 py-2 border-b border-gray-200 rounded-t-lg bg-gray-300"><span class="fa fa-user text-xs"> </span> Registro Log del flujo</li>
        ...
        <li style="max-height: 300px; overflow: scroll;">
            <table class="items-center w-full mb-0 align-top border-gray-200 text-slate-500">
                ...
                <tbody>
                @foreach($dyplogs as $dyplog)
                    <tr>
                        <td class="p-2 align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                            <div class="mb-0 leading-tight text-xs text-slate-400 text-xs"  style="white-space: pre-wrap"><span class="pl-2 fa {{($dyplog->Tipo == 'COMENTARIO')?'fa-comment-o text-green-700': (($dyplog->Tipo == 'ERROR')?'fa-times-circle text-red-700':'fa-info-circle text-sky-700')}}"> </span> {{$dyplog->textolog}}</div>
                        </td>
                        <td class="p-2 align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                            <p class="mb-0 font-semibold leading-tight text-xs" >{{$dyplog->Usuario->Email}}</p>
                            <p class="mb-0 leading-tight text-xs text-slate-400">({{$dyplog->created_at}})</p>
                        </td>
                    </tr>
                @endforeach
                </tbody>
