...
class="p-3">
    ...
    <ul class="w-100 text-xs font-mediartinez Sotoum text-gray-900 bg-white border border-gray-200 rounded-lg">
        <li class="w-full px-4 py-2 border-b border-gray-200 rounded-t-lg bg-gray-300"><span class="fa fa-user text-xs"> </span> Archivos subidos</li>
    ...
    <div class="flex flex-wrap w-full p-4">
        <form name="formArchivos" action="{{route('guardarArchivos')}}" method="post" id="formArchivos" class="w-full p-2 border border-gray-200 rounded-lg m-0 shadow-md" enctype="multipart/form-data">
        <input type="hidden" name="idDyp" id="idDyp" value="{{$idDyp}}">
            @csrf
        ...
        <div class="p-2 w-full">
            ...
            <input required id="input_Archivos" name="input_Archivos[]"  type="file" class=" bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder=""  multiple>
...
<script>
    ...
    function creaCPD(idVenta) {

        $.ajax({
            url: 'https://apps2.pompeyo.cl/api/creacpdventas',
            type: 'POST',
