<form name="formTarea" action="{{route('completartareacpd',[$tarea->ID])}}" method="post" id="formTarea" class=" p-2 border border-gray-200 rounded-lg m-0 shadow-md" enctype="multipart/form-data">

{{--
<form name="formTarea" wire:submit.prevent="completartareacpd" type="post" enctype="multipart/form-data">
--}}
    @csrf
    <div class="flex flex-wrap w-full">
        <div class="w-full sm:w-100 md:w-1/2 lg:w-2/3 xl:w-2/3 flex flex-wrap">
            ...
            @foreach($tarea->Tipo->Campos as $campo)
                @if($campo->Tipo == 'separador')
                    &nbsp;<p class="w-full  border-b-2 text-blue-500 text-bold">{{$campo->Campo}}</p>
                @else
                <div class="border border-light mb-3 p-2 border-gray-300 {{($campo->Agrupacion != null) ? $campo->Agrupacion : 'w-full'}} mb-4 ">
                    ...
                    <select id="{{$campo->InputName}}" name="{{$campo->InputName}}" class="{{$campo->Clase}} bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 " placeholder="{{$campo->Placeholder}}" {{($campo->Requerido) ? 'required' : ''}}>
...
@if($tarea->CpdTipoID == 3)
    <div class="border border-light mb-3 p-2 border-gray-300">
        <livewire:cpd.componentes.formulario-inspeccion : idCpd="{{$tarea->CpdID}}"/>
    </div>
@endif
@if($tarea->CpdTipoID == 13)
    <div class="border border-light mb-3 p-2 border-gray-300">
        <livewire:cpd.componentes.formulario-calidad : idCpd="{{$tarea->CpdID}}"/>
    </div>
@endif
...
<script>
    ...
    @foreach($tarea->Tipo->Campos as $campo)
        @if($campo->Clase != null)
            ...
            $('#{{$campo->Clase}}').on('click', function () {
                var seleccion = $('#{{$campo->Clase}}').val();
                console.log(seleccion);
                var url = '{{route('getoptions',[$campo->ID])}}';
                $.ajax({
                    type: 'GET',
                    url: url,
                    data: {seleccion: seleccion},
                    success: function (data) {
                        var options = '<option value="" disabled selected>Seleccione...</option>';
                        $.each(data, function (i, item) {
                            options += '<option value="' + item + '">' + item + '</option>';
                        });
                        $('#{{$campo->InputName}}').html(options);
                    },
                    error: function (data) {
