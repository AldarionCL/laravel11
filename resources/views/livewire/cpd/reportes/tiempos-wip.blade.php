<div class="flex flex-row gap-1 border-2 border-gray-300 p-2 rounded-lg content-start text-xs-center text-white overflow-auto">
    ...
    <div class="" style="min-width: 100px; width: 100%; z-index: 3;">
        ...
        @if(isset($arrayCasos[$paso->ID]))
            @foreach($arrayCasos[$paso->ID] as $caso)
                <a href="{{route('flujocpd',[$caso["ID"]])}}">
                    <div class="border-{{$caso["color"]}}-500 bg-gray-100 mt-2 text-xs text-gray-600 border-2  rounded-lg p-1 basis-1/9 shadow-md" style="font-size: 8px; max-width: 100px; margin: 0 auto; margin-bottom: 10px;">
                        <div class="text-center items-center m-0">
                            <div class="text-2xs">
