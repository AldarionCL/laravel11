<div class="flex flex-row gap-1 border-2 border-gray-300 p-2 rounded-lg content-start text-xs-center text-white overflow-auto">
    ...
    <div class="" style="min-width: 105px; width: 100%; z-index: 3;">
        <div class="{{$estados[$paso->Orden]}} border-2 border-blue-500 rounded-lg p-1 w-50 shadow-md mb-3">
            <div class="text-center items-center m-0">
                <span class=" fa {{$iconos[$paso->Orden]}} "></span>
                <br><small><small><small>{{$paso->Paso}}</small></small></small>
                <br>
        ...
        @if(isset($arrayCasos[$paso->Orden]))
            @foreach($arrayCasos[$paso->Orden] as $caso)
                <a href="{{route('flujocpd',[$caso["ID"]])}}">
                    <div class="border-{{$caso["color"]}}-500 bg-gray-100 mt-2 text-xs text-gray-600 border-2  rounded-lg p-1 w-50 basis-1/9 shadow-md" style="font-size: 8px; max-width: 75px; margin: 0 auto;  margin-bottom: 10px;">
                        <div class="text-center items-center m-0">
                            <div class="text-2xs">
                            ...
                            <div class="bg-white border-2 border-gray-900 text-2xs font-bold ">{{($caso["patente"])}}</div>
                            <br>
                            <div class="text-2xs">
                                Tiempo total:
