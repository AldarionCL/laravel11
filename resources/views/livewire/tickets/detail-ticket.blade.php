                                ...
                                <div
                                    ...
                                    @if($ticket->assigned === auth()->user()->ID && $ticket->state !== 3 )
                                        <div class="flex items-center">
                                           <div class="w-1/3">
                                               <form wire:submit.prevent="asignaResponsable">
                                               <x-simple-select wire:model="responsable" name="responsable" id="responsable" :options="$arrayResponsables" value-field='id' text-field='name' placeholder=""/>
                                               <button type="submit"
                                            ...
                                            <button onclick="Livewire.emit('openModal', 'ticket.reassignment-ticket', {{ json_encode([ 'id' => $ticket->id ]) }} )"
                                                    class="inline-block px-8 py-2 mb-4 ml-auto font-bold leading-normal text-center text-white align-middle transition-all ease-in bg-blue-500 border-0 rounded-lg shadow-md cursor-pointer text-size-xs tracking-tight-rem hover:shadow-xs hover:-translate-y-px active:opacity-85">
                                                <i class="fa fa-times-circle-o"></i>
                                                Re Asignación Ticket
                                            </button>
                                        ...
                                        <ul class="flex flex-col pl-0 mb-0 rounded-lg">
                                            <li class="relative flex p-6 mb-2 border-0 rounded-t-inherit rounded-xl bg-gray-50 dark:bg-slate-850">
                                                <div class="flex flex-col">
                                                    <span class="mb-2 leading-tight text-size-xs dark:text-white/80">Solicitante : <span
                                                            class="font-semibold text-slate-700 dark:text-white sm:ml-2">{{ $ticket->recorder->Nombre }}</span></span>
                                                    <span class="mb-2 leading-tight text-size-xs dark:text-white/80">Email Solicitante : <span
                                                            class="font-semibold text-slate-700 dark:text-white sm:ml-2">{{ $ticket->recorder->Email }}</span></span>
                                                    <span class="mb-2 leading-tight text-size-xs dark:text-white/80">Sucursal Solicitante : <span
                                                    ...
                                                    <span class="mb-2 leading-tight text-size-xs dark:text-white/80">Sucursal - Taller - Departamento : <span
                                                            class="font-semibold text-slate-700 dark:text-white sm:ml-2">{{ $ticket->office->Sucursal }}</span></span>
                                                    <span class="leading-tight text-size-xs dark:text-white/80">Identificador (<small>N° Venta, N° Ot, Factura, etc.</small>) : <span
                                                            class="font-semibold text-slate-700 dark:text-white sm:ml-2">{{ $ticket->identificador }}</span></span>
                                                </div>
                                            ...
                                            @if( $ticket->file )
                                                <li class="relative flex p-6 mb-2 border-0 rounded-t-inherit rounded-xl bg-gray-50 dark:bg-slate-850">
                                                    <div class="flex flex-row">
                                                        ...
                                                        <a href="{{ Storage::url( $ticket->file->url ) }}" target="_blank" class="ml-2">
                                    ...
                                    <livewire:ticket.comment-table ticket_id="{{ $ticket->id }}"/>
